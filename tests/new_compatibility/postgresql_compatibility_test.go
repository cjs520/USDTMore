...
func (suite *PostgreSQLCompatibilityTestSuite) TestPostgreSQLFieldTypeCompatibility() {
	...
	suite.Run("DecimalFieldReadWrite", func() {
		...
		order := &model.TradeOrders{
			...
			Amount:    func() decimal.Decimal { v, _ := decimal.NewFromString("100.12345678"); return v }(),
			Money:     func() decimal.Decimal { v, _ := decimal.NewFromString("690.88"); return v }(),
			Chain:     "TRON",
	...
	suite.Run("BooleanFieldMapping", func() {
		...
		// 验证 int16 字段正确映射
		assert.Equal(t, int16(1), retrievedAddress.OtherNotify)
		...
		assert.Equal(t, int16(0), retrievedAddress2.OtherNotify)
	})
	...
	suite.Run("NumericPrecisionAndScale", func() {
		...
		for i, amountStr := range testValues {
			...
			order := &model.TradeOrders{
				...
				UsdtRate:  decimal.NewFromFloat(7.0),
				Amount:    amount,
				Money:     func() decimal.Decimal { v, _ := decimal.NewFromString("100.50"); return v }(), // numeric(18,2)
				Chain:     "TRON",
				Address:   "TQn9Y2khEsLJW1ChVWFMSMeRDow5KcbLSE",
				Status:    1,
				Version:   0,
