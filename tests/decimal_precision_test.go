...
func (suite *DecimalPrecisionTestSuite) TestDecimalPrecisionMaintenance() {
	...
	suite.Run("AmountCalculationPrecision", func() {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				maxDiff := decimal.NewFromString("0.00000001") // 1e-8的精度差异
	...
	suite.Run("SafeFloatToDecimalConversion", func() {
		...
		for _, tf := range testFloats {
			...
			if tf.hasError {
				...
				// 允许微小的浮点精度差异
				diff := result.Sub(expected).Abs()
				maxDiff := decimal.NewFromString("0.000000001") // 1e-9
				assert.True(t, diff.LessThanOrEqual(maxDiff),
	...
	suite.Run("DatabaseRoundTripPrecision", func() {
		...
		for i, amountStr := range testAmounts {
			...
			order := &model.TradeOrders{
				...
				UsdtRate:  decimal.NewFromString("7.12345678"),
				Amount:    amount,
				Money:     decimal.NewFromString("100.50"),
				Chain:     "TRON",
	...
	suite.Run("AtomicPrecisionOperations", func() {
		...
		baseAmount := decimal.NewFromString("100.00")
		...
		for i := 0; i < 10; i++ {
			...
			assert.True(t, increment.Equal(expected) || increment.Sub(expected).Abs().LessThan(decimal.NewFromString("0.001")),
	...
	suite.Run("BoundaryValuePrecision", func() {
		...
		for _, valueStr := range boundaryValues {
			...
			order := &model.TradeOrders{
				...
				UsdtRate:  decimal.NewFromString("7.0"),
				Amount:    value,
				Money:     decimal.NewFromString("100.00"),
				Chain:     "TRON",
				Address:   "TQn9Y2khEsLJW1ChVWFMSMeRDow5KcbLSE",
				Status:    1,
				Version:   0,
...
func (suite *DecimalPrecisionTestSuite) TestDecimalAPIResponseFormat() {
	...
	suite.Run("APIResponseFormatConsistency", func() {
		...
		order := &model.TradeOrders{
			...
			UsdtRate:  decimal.NewFromString("6.87654321"),
			Amount:    decimal.NewFromString("123.12345678"),
			Money:     decimal.NewFromString("846.88"),
			Chain:     "TRON",
			Address:   "TQn9Y2khEsLJW1ChVWFMSMeRDow5KcbLSE",
			Status:    1,
			Version:   0,
		...
		moneyStr := help.FormatMoneyFixed(retrieved.Money)
	...
	suite.Run("CurrencyFormatConsistency", func() {
		...
		for _, tc := range testCases {
			...
			formatted := help.FormatMoneyFixed(money)
			assert.Equal(t, tc.expected, formatted,
...
func (suite *DecimalPrecisionTestSuite) TestDecimalCalculationAccuracy() {
	...
	suite.Run("ComplexCalculationPrecision", func() {
		...
		for _, scenario := range scenarios {
			t.Run(scenario.name, func(t *testing.T) {
				...
				// 验证计算精度
				diff := result.Sub(expected).Abs()
				maxDiff := decimal.NewFromString("0.00000001")
				assert.True(t, diff.LessThanOrEqual(maxDiff),
	...
	suite.Run("AccumulatedCalculationError", func() {
		// 测试多次小额计算是否会产生累积误差
		baseAmount := decimal.NewFromString("0.00000001")
